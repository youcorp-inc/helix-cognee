...
async def main():
    ...
    try:
        # Set HelixDB as the graph database provider
        graph_config = get_graph_config()
        graph_config.graph_database_url = "http://localhost"
        graph_config.graph_database_port = 6969

        cognee.config.set_graph_database_provider("helixdb")
        ...
        assert is_empty, "Helix graph database is not empty"

        await cognee.add([explanation_file_path_quantum], dataset_name)

        is_empty = await graph_engine.is_empty()

        assert is_empty, "Helix graph database should be empty before cognify"

        await cognee.cognify([dataset_name])

        is_empty = await graph_engine.is_empty()

        assert not is_empty, "Helix graph database should not be empty"
