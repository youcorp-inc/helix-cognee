...
async def main():
    ...
    data_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".data_storage/test_helixdb_vector")
        ).resolve()
    )
    cognee.config.data_root_directory(data_directory_path)
    cognee_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".cognee_system/test_helixdb_vector")
        ).resolve()
    )
    ...
    await cognee.prune.prune_system(metadata=True)
    tables_in_database = await vector_engine.get_all_collections()
    assert len(tables_in_database) == 0, "HelixDB database is not empty"
